...
if (isset($data['selectedDate']) && isset($data['selectedTime']) && isset($data['status']) && isset($data['remarks'])) {
    ...
    // Check if record for the selected date already exists
    $checkSql = "SELECT * FROM attendance WHERE date = '$selectedDate'";
    $result = $conn->query($checkSql);

    if ($result->num_rows > 0) {
        // Record already exists
        echo json_encode(array("message" => "Attendance already submitted for this date."));
    } else {
        // Insert new record
